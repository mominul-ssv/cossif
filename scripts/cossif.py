...
class CosSIF:
    ...
    def _create_temp_directories(self, t_path, s_path, save_path, image_size):
    ...
    def calculate_similarities(self, t_path, s_path, save_path, file_name, record_range=0, image_size=64):
        # check if the defined save path is correct 
        if os.path.exists(save_path)!=True:
            raise FileNotFoundError(f"Error! Path '{save_path}' does not exist!")
        ...
        temp_dir, t_dir, s_dir = self._create_temp_directories(t_path, s_path, save_path, image_size)
        ...
        np.save(os.path.join(save_path, file_name), np.array(set_R))
    ...
    def export_filtered_images(self, source_dir, filtered_dir, removed_dir, set_R, alpha, remove='similar'):
